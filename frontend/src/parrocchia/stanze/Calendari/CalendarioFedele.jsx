...
export default function CalendarioFedele({ parrocchia, onTorna }) {
  ...
  useEffect(() => {
    async function caricaEventi() {
      if (!parrocchia?.id) {
        setErrore("Parrocchia non identificata.");
  ...
  function categoriaEvento(evento) {
    ...
    if (origine.includes("catechismo")) return "catechismo";
  ...
  return (
    <div className="calendari-parroco">
      ...
      <div className="calendari-header">
        <div>
          <h1>Calendario della Parrocchia</h1>
          <p>
            Orari delle Messe ed eventi pubblici di {parrocchia?.nome}.
          </p>
        </div>
      </div>

      <div className="calendari-toolbar">
        <button type="button" onClick={mesePrecedente}>‹</button>
        <h2>{nomeMese}</h2>
        <button type="button" onClick={meseSuccessivo}>›</button>
        <button type="button" onClick={vaiAOggi}>Oggi</button>
      </div>
      ...
      <div className="calendari-filtri">
        ...
        {filtri.map(([valore, etichetta]) => (
          <button
            type="button"
            key={valore}
            className={filtroCategoria === valore ? "attivo" : ""}
            onClick={() => setFiltroCategoria(valore)}
          >
            {etichetta}
          </button>
        ))}
      ...
      {!caricamento && !errore && (
        <div className="calendario-contenitore">
            ...
            <div className="calendario-griglia">
              {giorniCalendario.map((giorno) => {
                ...
                return (
                  ...
                  >
                    <span className="numero-giorno">{giorno.getDate()}</span>
                    ...
                    <div className="eventi-giorno">
                      {eventiGiorno.slice(0, 4).map((evento) => (
                        ...
                        >
                          <span>{formattaOra(evento.data_ora_inizio)}</span>
          ...
          <aside className="calendario-dettaglio">
            <h3>{formattaGiornoCompleto(giornoSelezionato)}</h3>
